...
Everything marked **Fixed** is in `ff5e23f` and verified (tsc clean, 37 tests
pass, eslint unchanged at 199 problems / 4 errors — all pre-existing, all in
`docs/`). Everything marked **Proposed** needs a decision, a device, or both,
and is deliberately left uncommitted.
...
**Next release — the cutover.** S1 is the whole reason this release exists; do
it first, with the notes and the notice, and carry the fixes already committed
(S2, S3, S7, P1, P2, P6, P7, P8, P9, W1). If a data export is wanted ahead of
the cutover, it has to ship in the release *before* this one.
